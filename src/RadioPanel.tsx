    ...
    {open && <div className="radio-body" aria-busy={!loaded}>
      ...
      {settingsOpen && <div className="radio-setup">
        <fieldset disabled={!loaded || Boolean(busy) || Boolean(active)}>
          ...
          <label className="radio-memory-policy"><span>Memory check</span><select aria-label="Radio memory check" value={config.generation.wait_for_memory ? "wait" : "try"} onChange={event => editConfig({ generation: { ...config.generation, wait_for_memory: event.target.value === "wait" } })}><option value="wait">Wait for memory</option><option value="try">Try anyway</option></select><small>{config.generation.wait_for_memory ? "Waits until the estimated memory is available." : "Attempts rendering despite low memory. A failed take pauses the station."}</small></label><div className="radio-options-grid"><label><span>Song duration ceiling</span><select aria-label="Radio song duration" value={config.durationSeconds} onChange={event => editConfig({ durationSeconds: Number(event.target.value) })}>{![30, 45, 60, 90, 120, 180, 240, 360].includes(config.durationSeconds) && <option value={config.durationSeconds}>{config.durationSeconds} seconds</option>}{[30, 45, 60, 90, 120, 180, 240, 360].map(seconds => <option key={seconds} value={seconds}>{seconds < 60 ? `${seconds} seconds` : `${seconds / 60} ${seconds === 60 ? 'minute' : 'minutes'}`}</option>)}</select></label><label><span>Prepare ahead</span><select aria-label="Radio buffer ahead" value={config.bufferAhead} onChange={event => editConfig({ bufferAhead: Number(event.target.value) })}><option value={1}>1 song ahead</option><option value={2}>2 songs ahead</option></select></label><label><span>Songs this session</span><input aria-label="Radio session song limit" type="number" min={0} max={1000} step={1} value={config.maxTracksPerSession} onChange={event => editConfig({ maxTracksPerSession: Number(event.target.value) })} /><small>0 keeps the station going until Stop.</small></label><div className="radio-render-baseline"><span>RENDERING BASELINE</span><strong>{config.generation?.num_inference_steps || 32} steps · {config.generation?.cot === 'off' ? 'Direct' : config.generation?.cot === 'melody' ? 'Melody' : 'Melody + chords'}</strong><small>{config.generation?.weight_storage === 'native' ? 'Original storage' : 'Efficient Q8'} · {config.generation.wait_for_memory ? "Waits for memory" : "Tries with available memory"}</small></div></div>
        </fieldset>
